...
func TestBalancerPoolUpdatePoolAssetBalance(t *testing.T) {
	...
	pacc, err := balancer.NewBalancerPool(poolId, defaultBalancerPoolParams, initialAssets, defaultCurBlockTime)
	require.NoError(t, err)
...
func TestBalancerPoolAssetsWeightAndTokenBalance(t *testing.T) {
	...
	for i, tc := range tests {
		pacc, err := balancer.NewBalancerPool(poolId, defaultBalancerPoolParams, tc.assets, defaultCurBlockTime)
		if tc.shouldErr {
...
func TestGetBalancerPoolAssets(t *testing.T) {
	...
	pacc, err := balancer.NewBalancerPool(defaultPoolId, defaultBalancerPoolParams, assets, defaultCurBlockTime)
	require.NoError(t, err)
...
func TestLBPParamsEmptyStartTime(t *testing.T) {
	...
	_, err := balancer.NewBalancerPool(defaultPoolId, balancer.PoolParams{
		SwapFee: defaultSpreadFactor,
		ExitFee: defaultZeroExitFee,
	}, initialPoolAssets, defaultCurBlockTime)
	require.NoError(t, err)
}
...
func TestIsActive(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			// Initialize a pool
			pool, err := balancer.NewBalancerPool(defaultPoolId, defaultBalancerPoolParams, dummyPoolAssets, defaultCurBlockTime)
