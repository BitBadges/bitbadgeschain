...
import (
	...
	_ "github.com/bitbadges/bitbadgeschain/third_party/osmoutils"
...
func (s *KeeperTestSuite) TestCreateBalancerPool() {
	...
	tests := []struct {
	...
	}{
		{
			...
			msg:         balancer.NewMsgCreateBalancerPool(testAccount, defaultPoolParams, defaultPoolAssets),
			emptySender: false,
			expectPass:  true,
		}, {
	...
	for _, test := range tests {
		...
		if test.expectPass {
			...
			expectedSenderBal := senderBalBeforeNewPool.Sub(expectedPoolTokens...).Add(expectedPoolShares)
			// .Sub(params.PoolCreationFee...)
...
func (s *KeeperTestSuite) TestGetPoolDenom() {
	...
	poolCreateMsg := balancer.NewMsgCreateBalancerPool(s.TestAccs[0], defaultPoolParams, defaultPoolAssets)
