...
func (k Keeper) updatePoolForSwap(
	...
	err = k.SendCoinsToPoolWithAliasRouting(ctx, sender, poolAddress, sdk.Coins{
		tokenIn,
	})
	if err != nil {
		return err
	...
	if tokenOutToSender.IsPositive() {
		...
		err = k.SendCoinsFromPoolWithAliasRouting(ctx, pool.GetAddress(), sender, sdk.NewCoins(tokenOutCoinToSender))
		if err != nil {
			return err
		}
	}
	...
	if len(affiliates) > 0 && totalFeeAmount.IsPositive() {
		for i, affiliate := range affiliates {
			if affiliateFees[i].Amount.IsPositive() {
				...
				err = k.SendCoinsFromPoolWithAliasRouting(ctx, pool.GetAddress(), affiliateAddr, sdk.NewCoins(affiliateFees[i]))
				if err != nil {
