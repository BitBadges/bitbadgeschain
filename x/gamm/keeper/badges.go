		...
		if path.Denom == baseDenom {
			if path.DenomSuffixDetails != nil {
				if path.DenomSuffixDetails.WithAddress {
					return nil, fmt.Errorf("dynamic address is set: %s", denom)
				}

				if path.DenomSuffixDetails.DestinationCollectionId != "" {
					return nil, fmt.Errorf("destination collection ID is set: %s", denom)
				}

				if path.DenomSuffixDetails.DestinationChainId != "" {
					return nil, fmt.Errorf("destination chain ID is set: %s", denom)
				}
			}
			return path, nil
		}
