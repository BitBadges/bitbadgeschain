...
func (suite *TwoFADecoratorTestSuite) SetupTest() {
	...
	// Setup twofa keeper with its own store
	// For testing, we'll use a separate store key but same codec and logger
	storeKey := storetypes.NewKVStoreKey(twofatypes.StoreKey)
	stateStore := suite.ctx.MultiStore().(storetypes.CommitMultiStore)
	stateStore.MountStoreWithDB(storeKey, storetypes.StoreTypeIAVL, nil)
	suite.Require().NoError(stateStore.LoadLatestVersion())
	
	// Create codec for twofa (register interfaces)
	// Use the same codec setup as badges keeper for compatibility
	// We need to register both twofa and badges interfaces since twofa uses badges types
	registry := codectypes.NewInterfaceRegistry()
	twofatypes.RegisterInterfaces(registry)
	types.RegisterInterfaces(registry)
	appCodec := codec.NewProtoCodec(registry)
	
	suite.twofaKeeper = NewKeeper(
		appCodec,
	...
	// Setup decorator
	suite.decorator = NewTwoFADecorator(suite.twofaKeeper)
...
func (suite *TwoFADecoratorTestSuite) set2FARequirements(address sdk.AccAddress, requirements []*types.MustOwnTokens) {
	...
	// After generation, types.User2FARequirements will be available from proto/twofa/user_2fa_requirements.proto
	user2FARequirements := &twofatypes.User2FARequirements{
		MustOwnTokens:          requirements,
		DynamicStoreChallenges: []*types.DynamicStoreChallenge{},
	}
	err := suite.twofaKeeper.SetUser2FARequirementsInStore(suite.ctx, address.String(), user2FARequirements)
	suite.Require().NoError(err)
}
...
func (suite *TwoFADecoratorTestSuite) Test2FAFailure() {
	...
	requirements, found := suite.twofaKeeper.GetUser2FARequirementsFromStore(suite.ctx, suite.alice.String())
