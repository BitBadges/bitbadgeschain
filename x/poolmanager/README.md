...
### Context-local fee state

Fee-share agreements, registered alloyed pools, and the default taker fee are
read from the caller's SDK store. Discarding a CacheContext must discard its fee
changes too; process-global caches cannot safely represent consensus state.
