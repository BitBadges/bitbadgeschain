...
func GetCollection(suite *TestSuite, ctx context.Context, id sdkmath.Uint) (*types.TokenCollection, error) {
	res, err := suite.app.TokenizationKeeper.GetCollection(ctx, &types.QueryGetCollectionRequest{CollectionId: sdkmath.Uint(id).String()})
	if err != nil {
...
func GetUserBalance(suite *TestSuite, ctx context.Context, collectionId sdkmath.Uint, address string) (*types.UserBalanceStore, error) {
	res, err := suite.app.TokenizationKeeper.GetBalance(ctx, &types.QueryGetBalanceRequest{
...
func GetAddressList(suite *TestSuite, ctx context.Context, listId string) (*types.AddressList, error) {
	res, err := suite.app.TokenizationKeeper.GetAddressList(ctx, &types.QueryGetAddressListRequest{
		ListId: listId,
	})
	if err != nil {
...
// 	res, err := suite.app.TokenizationKeeper.GetChallengeTracker(ctx, &types.QueryGetChallengeTrackerRequest{
...
// 	res, err := suite.app.TokenizationKeeper.GetApprovalTracker(ctx, &types.QueryGetApprovalTrackerRequest{
...
func MintAndDistributeTokens(suite *TestSuite, ctx context.Context, msg *types.MsgMintAndDistributeTokens) error {
	...
	for _, transfer := range msg.Transfers {
		...
		newTransfer.PrioritizedApprovals = GetDefaultPrioritizedApprovals(sdk.UnwrapSDKContext(ctx), suite.app.TokenizationKeeper, msg.CollectionId)
