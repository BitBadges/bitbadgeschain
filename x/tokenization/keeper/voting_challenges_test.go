...
func (suite *TestSuite) TestVotingChallenge_MultipleVoters() {
	...
	aliceVote, found := suite.app.TokenizationKeeper.GetVoteFromStore(suite.ctx, aliceVoteKey)
	suite.Require().True(found, "Alice's vote should be stored")
	suite.Require().Equal(sdkmath.NewUint(100), aliceVote.YesWeight)

	bobVote, found := suite.app.TokenizationKeeper.GetVoteFromStore(suite.ctx, bobVoteKey)
	suite.Require().True(found, "Bob's vote should be stored")
	suite.Require().Equal(sdkmath.NewUint(50), bobVote.YesWeight)

	charlieVote, found := suite.app.TokenizationKeeper.GetVoteFromStore(suite.ctx, charlieVoteKey)
...
func (suite *TestSuite) TestQueryVote() {
	...
	// Query the vote
	res, err := suite.app.TokenizationKeeper.GetVote(wctx, &types.QueryGetVoteRequest{
		CollectionId:    "1",
		ApprovalLevel:   "collection",
		ApproverAddress: "",
		ApprovalId:      "test",
...
func (suite *TestSuite) TestQueryVotes() {
	...
	res, err := suite.app.TokenizationKeeper.GetVotes(wctx, &types.QueryGetVotesRequest{
		CollectionId:    "1",
		ApprovalLevel:   "collection",
		ApproverAddress: "",
		ApprovalId:      "test",
...
func (suite *TestSuite) TestVotingChallenge_VoterSetCompleteChange() {
	...
	testVersion, found := suite.app.TokenizationKeeper.GetApprovalTrackerVersionFromStore(suite.ctx, keeper.ConstructApprovalVersionKey(sdkmath.NewUint(1), "collection", "", "test"))
