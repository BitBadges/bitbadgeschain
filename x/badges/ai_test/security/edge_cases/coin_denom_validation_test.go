...
func (suite *CoinDenomValidationTestSuite) SetupTest() {
	...
	updateMsg := &badgestypes.MsgUniversalUpdateCollection{
		...
		CollectionApprovals:       []*badgestypes.CollectionApproval{collectionApproval},
	}
...
func getFullUintRanges() []*badgestypes.UintRange {
	return []*badgestypes.UintRange{
		{
...
func (suite *CoinDenomValidationTestSuite) TestCoinDenomValidation_MultiCoinUnauthorizedDenom() {
	...
	// Mint badges to Bob
	suite.MintBadges(collectionId, suite.Bob, []*badgestypes.Balance{
		{
	...
	updateMsg := &badgestypes.MsgUpdateUserApprovals{
		Creator:                 suite.Bob,
		...
		OutgoingApprovals: []*badgestypes.UserOutgoingApproval{
			{
				...
				ApprovalCriteria: &badgestypes.OutgoingApprovalCriteria{
					MaxNumTransfers: &badgestypes.MaxNumTransfers{
						OverallMaxNumTransfers: sdkmath.NewUint(1000),
						AmountTrackerId:        "test-tracker",
					},
					ApprovalAmounts: &badgestypes.ApprovalAmounts{
						PerFromAddressApprovalAmount: sdkmath.NewUint(1),
						AmountTrackerId:              "test-tracker",
					},
					CoinTransfers: []*badgestypes.CoinTransfer{
						{
							To: suite.Alice,
							Coins: []*sdk.Coin{
	...
	setIncomingMsg := &badgestypes.MsgSetIncomingApproval{
		Creator:      suite.Alice,
		CollectionId: collectionId,
	...
	transferMsg := &badgestypes.MsgTransferTokens{
		Creator:      suite.Alice,
		CollectionId: collectionId,
