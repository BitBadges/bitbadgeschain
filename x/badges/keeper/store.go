...
func (k Keeper) SetClaimInStore(ctx sdk.Context, claimId uint64, Claim types.Claim) error {
	marshaled_claim, err := k.cdc.Marshal(&Claim)
	if err != nil {
		return sdkerrors.Wrap(err, "Marshal types.Claim failed")
	}

	store := ctx.KVStore(k.storeKey)
	store.Set(claimStoreKey(claimId), marshaled_claim)
	return nil
}
...
func (k Keeper) GetClaimFromStore(ctx sdk.Context, claimId uint64) (types.Claim, bool) {
	...
	if len(marshaled_claim) == 0 {
		return Claim, false
	}
	k.cdc.MustUnmarshal(marshaled_claim, &Claim)
	return Claim, true
}

// GetClaimsFromStore defines a method for returning all user balances information by key.
func (k Keeper) GetClaimsFromStore(ctx sdk.Context) (addresses []*types.Claim) {
	store := ctx.KVStore(k.storeKey)
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		addresses = append(addresses, &Claim)
	}
	return
}
...
func (k Keeper) StoreHasClaim(ctx sdk.Context, claimId uint64) bool {
	store := ctx.KVStore(k.storeKey)
	return store.Has(claimStoreKey(claimId))
}
