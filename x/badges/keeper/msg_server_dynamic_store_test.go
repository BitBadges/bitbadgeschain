...
func TestDynamicStoreFlow(t *testing.T) {
	...
	queryResp, err := suite.app.BadgesKeeper.GetDynamicStore(wctx, &types.QueryGetDynamicStoreRequest{
		StoreId: "1",
	})
	require.NoError(t, err)
	...
	// Verify the deletion
	_, err = suite.app.BadgesKeeper.GetDynamicStore(wctx, &types.QueryGetDynamicStoreRequest{
		StoreId: "1",
	})
	require.Error(t, err)
