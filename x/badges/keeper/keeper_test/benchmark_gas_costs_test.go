...
func (suite *TestSuite) TestGasCosts() {
	...
	for i := 0; i < 1; i++ {
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		_, _ = GetUserBalance(suite, wctx, 0, 0, firstAccountNumCreated+1)
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		// err = HandlePendingTransfers(suite, wctx, alice, true, 0, []*types.IdRange{{Start: 0, End: 0}}, 0)
		// suite.Require().Nil(err, "Error accepting badge")
		...
		// err = HandlePendingTransfers(suite, wctx, alice, true, 0, []*types.IdRange{{Start: 0, End: 999}}, 0)
		// suite.Require().Nil(err, "Error transferring badge")
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		_, _ = GetUserBalance(suite, wctx, 0, 0, firstAccountNumCreated)
		endGas = suite.ctx.GasMeter().GasConsumed()
		tbl.AddRow("GetUserBalance", endGas-startGas)
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		userBalanceInfo, _ := GetUserBalance(suite, wctx, 1, 0, firstAccountNumCreated)
		endGas = suite.ctx.GasMeter().GasConsumed()
		tbl.AddRow("GetUserBalance", endGas-startGas)
