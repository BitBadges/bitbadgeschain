...
func (suite *TestSuite) TestGasCosts() {
	...
	for i := 0; i < 1; i++ {
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		err = FreezeAddresses(suite, wctx, bob, []*types.NumberRange{{Start: 1000, End: 1000}}, 0, 0, true)
		suite.Require().Nil(err, "Error creating subbadge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		err = FreezeAddresses(suite, wctx, bob, []*types.NumberRange{{Start: 0, End: 9999}}, 0, 0, true)
		suite.Require().Nil(err, "Error creating subbadge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		err = FreezeAddresses(suite, wctx, bob, []*types.NumberRange{{Start: 1000, End: 1000}}, 0, 0, false)
		suite.Require().Nil(err, "Error creating subbadge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		err = FreezeAddresses(suite, wctx, bob, []*types.NumberRange{{Start: 0, End: 9999}}, 0, 0, false)
		suite.Require().Nil(err, "Error creating subbadge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		err = TransferBadge(suite, wctx, bob, firstAccountNumCreated, []uint64{firstAccountNumCreated + 1}, []uint64{1}, 0, []*types.NumberRange{{Start: 0, End: 0}}, 0)
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		err = TransferBadge(suite, wctx, bob, firstAccountNumCreated, []uint64{firstAccountNumCreated + 1}, []uint64{1}, 0, []*types.NumberRange{{Start: 0, End: 999}}, 0)
		suite.Require().Nil(err, "Error transferring badge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		err = TransferBadge(suite, wctx, bob, firstAccountNumCreated, []uint64{firstAccountNumCreated + 1}, []uint64{1}, 0, []*types.NumberRange{{Start: 1000, End: 1999}}, 0)
		suite.Require().Nil(err, "Error transferring badge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		err = RequestTransferBadge(suite, wctx, alice, firstAccountNumCreated, 1, 0, []*types.NumberRange{{Start: 0, End: 0}}, 0)
		suite.Require().Nil(err, "Error transferring badge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		// err = HandlePendingTransfers(suite, wctx, alice, true, 0, []*types.NumberRange{{Start: 0, End: 0}}, 0)
		// suite.Require().Nil(err, "Error accepting badge")
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		err = RequestTransferBadge(suite, wctx, alice, firstAccountNumCreated, 1, 0, []*types.NumberRange{{Start: 1000, End: 1999}}, 0)
		suite.Require().Nil(err, "Error transferring badge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		// err = HandlePendingTransfers(suite, wctx, alice, true, 0, []*types.NumberRange{{Start: 0, End: 999}}, 0)
		// suite.Require().Nil(err, "Error transferring badge")
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		err = RequestTransferBadge(suite, wctx, alice, firstAccountNumCreated, 1, 0, []*types.NumberRange{{Start: 0, End: 999}}, 0)
		suite.Require().Nil(err, "Error transferring badge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		tbl.AddRow("Request Transfer- 1000", endGas-startGas)
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		err = RevokeBadges(suite, wctx, bob, []uint64{firstAccountNumCreated + 1}, []uint64{1}, 0, []*types.NumberRange{{Start: 0, End: 0}})
		suite.Require().Nil(err, "Error transferring badge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		err = RevokeBadges(suite, wctx, bob, []uint64{firstAccountNumCreated + 1}, []uint64{1}, 0, []*types.NumberRange{{Start: 1000, End: 1999}})
		suite.Require().Nil(err, "Error transferring badge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		err = SetApproval(suite, wctx, bob, 10000, firstAccountNumCreated+1, 0, []*types.NumberRange{{Start: 0, End: 0}}, 0)
		suite.Require().Nil(err, "Error transferring badge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		err = SetApproval(suite, wctx, bob, 10, firstAccountNumCreated+1, 0, []*types.NumberRange{{Start: 0, End: 999}}, 0)
		suite.Require().Nil(err, "Error transferring badge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		err = TransferBadge(suite, wctx, bob, firstAccountNumCreated, []uint64{firstAccountNumCreated + 1}, []uint64{1}, 1, []*types.NumberRange{{Start: 0, End: 0}}, 0)
		suite.Require().Nil(err, "Error transferring badge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		err = TransferBadge(suite, wctx, bob, firstAccountNumCreated, []uint64{firstAccountNumCreated + 1}, []uint64{1}, 1, []*types.NumberRange{{Start: 0, End: 999}}, 0)
		suite.Require().Nil(err, "Error transferring badge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		err = TransferBadge(suite, wctx, bob, firstAccountNumCreated, addresses, []uint64{1}, 1, []*types.NumberRange{{Start: 0, End: 0}}, 0)
		suite.Require().Nil(err, "Error transferring badge")
		suite.Require().Nil(err, "Error transferring badge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		err = TransferBadge(suite, wctx, bob, firstAccountNumCreated, addresses, []uint64{1}, 1, []*types.NumberRange{{Start: 0, End: 999}}, 0)
		suite.Require().Nil(err, "Error transferring badge")
		suite.Require().Nil(err, "Error transferring badge")
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		for i := uint64(0); i < 1000; i++ {
			err = TransferBadge(suite, wctx, bob, firstAccountNumCreated, []uint64{firstAccountNumCreated + 1}, []uint64{1}, 1, []*types.NumberRange{{Start: i * 2, End: i * 2}}, 0)
			suite.Require().Nil(err, "Error transferring badge")
			suite.Require().Nil(err, "Error transferring badge")
		}
		endGas = suite.ctx.GasMeter().GasConsumed()
		...
		startGas = suite.ctx.GasMeter().GasConsumed()
		badgeBalanceInfo, _ := GetBadgeBalance(suite, wctx, 1, 0, firstAccountNumCreated)
		endGas = suite.ctx.GasMeter().GasConsumed()
		tbl.AddRow("GetBadgeBalance", endGas-startGas)
