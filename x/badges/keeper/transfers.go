...
func (k Keeper) HandleTransfers(ctx sdk.Context, collection *types.BadgeCollection, transfers []*types.Transfer, initiatedBy string) error {
	...
	for _, transfer := range transfers {
		...
		for _, to := range transfer.ToAddresses {
			...
			fromUserBalance, toUserBalance, err = k.HandleTransfer(ctx, collection, transfer.Balances, fromUserBalance, toUserBalance, transfer.From, to, initiatedBy, transfer.MerkleProofs, transfer.PrioritizedApprovals, transfer.OnlyCheckPrioritizedApprovals, transfer.ZkProofSolutions)
			if err != nil {
				return err
			}
...
func (k Keeper) HandleTransfer(ctx sdk.Context, collection *types.BadgeCollection, transferBalances []*types.Balance, fromUserBalance *types.UserBalanceStore, toUserBalance *types.UserBalanceStore, from string, to string, initiatedBy string, solutions []*types.MerkleProof, prioritizedApprovals []*types.ApprovalIdentifierDetails, onlyCheckPrioritized bool, zkProofSolutions []*types.ZkProofSolution) (*types.UserBalanceStore, *types.UserBalanceStore, error) {
	err := *new(error)
	...
	for _, balance := range transferBalances {
		...
		userApprovals, err := k.DeductCollectionApprovalsAndGetUserApprovalsToCheck(ctx, transferBalances, collection, badgeIds, times, from, to, initiatedBy, amount, solutions, prioritizedApprovals, onlyCheckPrioritized, zkProofSolutions)
		if err != nil {
			...
			for _, userApproval := range userApprovals {
				for _, balance := range userApproval.Balances {
					if userApproval.Outgoing {
						err = k.DeductUserOutgoingApprovals(ctx, transferBalances, collection, fromUserBalance, balance.BadgeIds, balance.OwnershipTimes, from, to, initiatedBy, amount, solutions, prioritizedApprovals, onlyCheckPrioritized, zkProofSolutions)
						if err != nil {
							return &types.UserBalanceStore{}, &types.UserBalanceStore{}, sdkerrors.Wrapf(err, "outgoing approvals for %s not satisfied", from)
						}
					} else {
						err = k.DeductUserIncomingApprovals(ctx, transferBalances, collection, toUserBalance, balance.BadgeIds, balance.OwnershipTimes, from, to, initiatedBy, amount, solutions, prioritizedApprovals, onlyCheckPrioritized, zkProofSolutions)
						if err != nil {
