...
func TestKeeper_GetWrappableBalances(t *testing.T) {
		...
		Transfers: []*types.Transfer{
			{
				...
				PrioritizedApprovals: GetDefaultPrioritizedApprovals(suite.ctx, suite.app.BadgesKeeper, sdkmath.NewUint(4)),
			},
		},
	})
	require.NoError(t, err, "error minting and distributing badges to bob for amount>1 test")

	respAmount, err := suite.app.BadgesKeeper.GetWrappableBalances(wctx, &types.QueryGetWrappableBalancesRequest{
	...
	nextIdNoWrap := suite.app.BadgesKeeper.GetNextCollectionId(suite.ctx)
		...
		Transfers: []*types.Transfer{
			{
				...
				PrioritizedApprovals: GetDefaultPrioritizedApprovals(suite.ctx, suite.app.BadgesKeeper, nextIdNoWrap),
			},
		},
	})
					...
					Transfers: []*types.Transfer{
						{
							...
							PrioritizedApprovals: GetDefaultPrioritizedApprovals(suite.ctx, suite.app.BadgesKeeper, nextId),
						},
					},
				})
