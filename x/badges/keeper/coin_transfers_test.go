...
func (suite *TestSuite) TestUserLevelRestrictions() {
	collectionsToCreate := GetTransferableCollectionToCreateAllMintedToCreator(bob)

	wctx := sdk.WrapSDKContext(suite.ctx)
	collectionsToCreate[0].CollectionApprovals[1].ApprovalCriteria.UserLevelRestrictions = &types.UserLevelRestrictions{
	...
	// Test that the collection was created with the correct userLevelRestrictions
	...
	suite.Require().NotNil(collection.CollectionApprovals[1].ApprovalCriteria.UserLevelRestrictions, "userLevelRestrictions should not be nil")
	suite.Require().False(collection.CollectionApprovals[1].ApprovalCriteria.UserLevelRestrictions.AllowAllDenoms, "allowAllDenoms should be false")
	suite.Require().Equal([]string{"ubadge", "uatom"}, collection.CollectionApprovals[1].ApprovalCriteria.UserLevelRestrictions.AllowedDenoms, "allowedDenoms should match")
	...
	suite.Require().True(found, "collection should exist")
	suite.Require().NotNil(collection.CollectionApprovals[1].ApprovalCriteria.UserLevelRestrictions, "userLevelRestrictions should not be nil")
	suite.Require().True(collection.CollectionApprovals[1].ApprovalCriteria.UserLevelRestrictions.AllowAllDenoms, "allowAllDenoms should be true")
	suite.Require().Empty(collection.CollectionApprovals[1].ApprovalCriteria.UserLevelRestrictions.AllowedDenoms, "allowedDenoms should be empty when allowAllDenoms is true")
}
